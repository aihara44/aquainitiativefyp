...
$(document).ready(function() {
	...
	manageProductTable = $('#table_ppukm').DataTable({
		'ajax': 'php_action/fetchppukm.php',
		'order': []
	});
    customer_id
		// // product form reset
		...
		$("#productImage").fileinput({
	      overwriteInitial: true,
		    ...
		    layoutTemplates: {main2: '{preview} {remove} {browse}'},								    
	  		allowedFileExtensions: ["jpg", "png", "gif", "pdf", "JPG", "PNG", "GIF", "PDF"]
			});   
		...
		$("#submitProductForm").unbind('submit').bind('submit', function() {
			...
			var unit = $("#unit").val();
			var block = $("#block").val();
			var dpquota = $("#dpquota").val();
			var full_bottle = $("#full_bottle").val();
            var empty_bottle = $("#empty_bottle").val();
            var date = $("#date").val();
	
			if(productImage == "") {
				$("#productImage").closest('.center-block').after('<p class="text-danger">Receipt Image field is required</p>');
			...
			if(department == "") {
				$("#department").after('<p class="text-danger">Department Name field is required</p>');
			...
			if(unit == "") {
				$("#unit").after('<p class="text-danger">Department Unit field is required</p>');
			...
			if(block == "") {
				$("#block").after('<p class="text-danger">Department Block field is required</p>');
				$('#block').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#block").find('.text-danger').remove();
				// success out for form 
				$("#block").closest('.form-group').addClass('has-success');	  	
			}	// /else

			if(dpquota == "") {
				$("#dpquota").after('<p class="text-danger">Department Monthly Quota field is required</p>');
				$('#dpquota').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#dpquota").find('.text-danger').remove();
				// success out for form 
				$("#dpquota").closest('.form-group').addClass('has-success');	  	
			}	// /else

			if(full_bottle == "") {
				$("#full_bottle").after('<p class="text-danger">Delivered Full Bottle field is required</p>');
				$('#full_bottle').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#full_bottle").find('.text-danger').remove();
				// success out for form 
				$("#full_bottle").closest('.form-group').addClass('has-success');	  	
			}	// /else
            
            if(empty_bottle == "") {
				$("#empty_bottle").after('<p class="text-danger">Collected Empty Bottle field is required</p>');
				$('#empty_bottle').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#empty_bottle").find('.text-danger').remove();
				// success out for form 
				$("#empty_bottle").closest('.form-group').addClass('has-success');	  	
			}	// /else
			...
			if(productImage && customer_id && department && unit && block && dpquota && full_bottle && empty_bottle && date) {
...
function editProduct(productId = null) {
	...
	if(productId) {
		...
		$.ajax({
			url: 'php_action/fetchSelectedPpukm.php',
			type: 'post',
			...
			// alert(response.product_image);
				...
				$("#getProductImage").attr('src', 'stock/'+response.resit);
		  ...
		  // 		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
				...
				// product id 
				$(".editProductFooter").append('<input type="hidden" name="productId" id="productId" value="'+response.gallon_id+'" />');				
				$(".editProductPhotoFooter").append('<input type="hidden" name="productId" id="productId" value="'+response.gallon_id+'" />');				
				
				// product name
				$("#customer_id").val(response.customer_id);
				// quantity
				$("#full_bottle").val(response.botol_penuh);
				// rate
				$("#empty_bottle").val(response.botol_kosong);
